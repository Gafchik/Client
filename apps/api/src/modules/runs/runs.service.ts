...
export class RunsService implements OnModuleInit {
        ...
        if (plan.roles.analyst.enabled) {
          ...
          if (clarificationQuestions.length) {
            await this.requestClarification(
              ...
              String((spec as any)?.description || 'Аналитик просит уточнить детали задачи перед продолжением.'),
            );
            return;
          }
  ...
  private buildDeveloperStatus(plan: NormalizedExecutionPlan, runMode: RunMode): string {
    ...
    if (!plan.roles.analyst.enabled) return 'Работаю напрямую по задаче и только по реальному коду';

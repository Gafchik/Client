...
interface ApprovalRequest {
  ...
  kind: 'command' | 'migration' | 'file_write' | 'clarification';
  role: ExecutionRole;
  /** Короткий заголовок — что именно собирается сделать агент. */
  title: string;
  /** Что делает действие — развёрнутое описание одним абзацем. */
  description: string;
  /** Для уточняющих вопросов: список конкретных вопросов пользователю. */
  questions?: string[];
...
type ExecutionRole = 'pm' | 'developer' | 'reviewer' | 'tester';
...
export class RunsService implements OnModuleInit {
  ...
  private async requestClarification(
    ...
    questions: string[],
    description?: string,
  ): Promise<{ answered: boolean; answer: string; approvalId: string }> {
    const normalizedQuestions = questions.map((item) => String(item || '').trim()).filter(Boolean);
    if (!normalizedQuestions.length) {
      return { answered: false, answer: '', approvalId: '' };
    }
    ...
    const approval: ApprovalRequest = {
      ...
      role,
      title,
      description: description || 'Нужен короткий ответ пользователя, чтобы продолжить работу без догадок.',
    ...
    await this.appendRunEvent(runId, 'approval:requested', approval);
    await this.broadcastActivity(runId, chatId, role, name, label, 'working', `Нужно уточнение от пользователя: ${approval.title}`);
    await this.runRepo.update(runId, { status: 'waiting_approval' });
    ...
    while (Date.now() - started < 30 * 60 * 1000) {
      ...
      if (!current) break;
      if (current.status === 'approved') {
        const answer = String(current.reason || '').trim();
        await this.broadcastActivity(runId, chatId, role, name, label, 'done', answer ? `Уточнение получено: ${approval.title}` : `Уточнение подтверждено без текста: ${approval.title}`);
        return { answered: true, answer, approvalId: approval.id };
      }
      if (current.status === 'rejected') {
        await this.broadcastActivity(runId, chatId, role, name, label, 'error', `Уточнение отклонено: ${approval.title}`);
        return { answered: false, answer: String(current.reason || '').trim(), approvalId: approval.id };
      }
      if (run?.status === 'cancelled' || run?.status === 'paused') {
        return { answered: false, answer: '', approvalId: approval.id };
      }
    }

    await this.broadcastActivity(runId, chatId, role, name, label, 'error', `Истекло ожидание уточнения: ${approval.title}`);
    return { answered: false, answer: '', approvalId: approval.id };
  }
  ...
  async executeRunSteps(runId: string): Promise<void> {
    ...
    const pmAgent = agents.pm;
    const developerAgent = agents.developer;
    const testerAgent = agents.tester;

    if (!pmAgent?.model) {
      throw new Error('Project Manager model is not configured');
    }

    // Имена агентов
    const pmName = pmAgent.name || 'Alex';
    const pmLabel = pmAgent.label || 'Project Manager';
    const devName = developerAgent.name || 'Kai';
    const devLabel = developerAgent.label || 'Разработчик';
    const testName = testerAgent.name || 'Nova';
    const testLabel = testerAgent.label || 'Верификатор';
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      if (attempt > 1) {
        await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'working', `Повторная попытка ${attempt}/${MAX_RETRIES}...`);
      } else {
        await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'working', 'Анализирую задачу и планирую работу команды');
      }
      ...
      try {
        // 1. PROJECT MANAGER - планирует работу.
        // ВАЖНО: повторно НЕ стримим ответ PM в чат — читаемый ответ
        // уже сохранён в chats.service.sendMessageToOrchestrator. Здесь PM
        // вызывается только чтобы построить план для разработчика.
        // Повторный token:stream создавал второй пузырь текста в чате (дубль).
        const pmPlanResult = await this.callAgentStream(
          runId, chatId, 'pm', pmAgent, language,
        ...
        if (!pmPlanResult.success) {
          if (pmPlanResult.rawResponse) {
            const extracted = this.extractPlanFromText(pmPlanResult.rawResponse);
            if (extracted) {
              pmPlanResult.success = true;
              pmPlanResult.artifact = extracted;
            }
          }
        }

        if (!pmPlanResult.success) {
          throw new Error(`Project Manager planning failed: ${pmPlanResult.error}`);
        }

        const rawPlan = (pmPlanResult.artifact || {}) as Record<string, unknown>;
        ...
        let pmWorked = false;
        if (plan.roles.pm.enabled) {
          if (!pmAgent?.model) {
            throw new Error('Project Manager role is required by the plan, but its model is not configured');
          }

          await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'working', this.buildPmStatus(plan, runMode));

          const pmResult = await this.callAgentStream(
            runId, chatId, 'pm', pmAgent, language,
            this.buildProjectManagerPrompt(run, plan, project, chat.messages, projectPath, workspace, runMode, memoryContext),
            (delta) => this.wsGateway.broadcastTokenStream(chatId, { role: 'pm', content: delta, done: false })
          );

          if (!pmResult.success && pmResult.rawResponse) {
            const fixed = this.tryFixAgentJson(pmResult.rawResponse, 'pm');
            if (fixed) {
              pmResult.success = true;
              pmResult.artifact = fixed;
            }
          }

          if (!pmResult.success) {
            throw new Error(`Project Manager failed: ${pmResult.error}`);
          }

          spec = (pmResult.artifact || {}) as Record<string, unknown>;
          pmWorked = true;
          await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'done', this.buildPmDoneStatus(plan, runMode));
          // Сохраняем ТЗ PM как сообщение в чат, чтобы оно персистило
          const pmRawText = String(pmResult.rawResponse || '').trim();
          if (pmRawText) {
            await this.chatsService.addMessage(chatId, 'assistant', pmRawText, {
              type: 'agent-brief',
              role: 'pm',
              name: pmName,
              label: pmLabel,
          ...
          const clarificationQuestions = Array.isArray((spec as any)?.clarification_questions)
            ? (spec as any).clarification_questions.map((item: any) => String(item || '').trim()).filter(Boolean)
            : [];
          if (clarificationQuestions.length) {
            await this.requestClarification(
              ...
              pmName,
              pmLabel,
              'Нужно уточнить требования перед реализацией',
              clarificationQuestions,
              String((spec as any)?.description || 'Project Manager просит уточнить детали задачи перед продолжением.'),
            );
            return;
          }
          await this.saveProjectMemory(projectId, chatId, spec, language, runMode, runId);
        } else {
          await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'done', `Этап пропущен: ${plan.roles.pm.reason}`);
        }
        ...
        if (plan.roles.developer.enabled) {
            ...
            if (Array.isArray(devSpec.files)) {
              devSpec.files = devSpec.files
                .map((file: any) => {
                  ...
                  const absolutePath = this.resolveToAbsolutePath(projectPath, originalPath);
                  if (!absolutePath || hasSuspiciousMirroredPath(absolutePath)) return null;
                  const parentPath = path.dirname(absolutePath);
                  const action = String(file?.action || '').trim().toLowerCase();
                  const exists = fs.existsSync(absolutePath) || fs.existsSync(parentPath);
          ...
          try {
            ...
            for (const f of devFiles) {
              ...
              const fullPath = this.resolveToAbsolutePath(projectPath, f.path);
          ...
          for (const request of requestedCommands) {
            const cwd = request.cwd ? this.resolveToAbsolutePath(projectPath, request.cwd) : projectPath;
            const suspiciousNestedCwd =
              request.cwd &&
              cwd !== projectPath &&
              /^(apps|packages|services|libs)\/[^/]+$/i.test(String(request.cwd)) &&
        ...
        // и т.п.) — ПИШЕМ в репо, если это запрошено задачей, и дополнительно
        // сохраняем контент в project memory для дальнейшей синхронизации знаний.
        ...
        if (files && Array.isArray(files) && files.length) {
          if (isDiagnostics) {
            ...
            for (const fileChange of files) {
              // Документацию теперь тоже применяем в репо, если она запрошена
              // задачей (удаление/создание/обновление .md, README, docs/*).
              // При этом сохраняем копию контента в project memory как артефакт.
              if (this.isDocumentationPath(fileChange.path)) {
                await this.broadcastActivity(runId, chatId, 'developer', devName, devLabel, 'working', `Документация «${fileChange.path}» — обновляю в репо и синхронизирую в память проекта`);
          ...
          if (requestedTests.length) {
            for (const test of requestedTests) {
              ...
              const result = await this.executeCommandWithApproval(
                ...
                test.output || `Верификатор хочет выполнить ${test.command}`,
                teamConfig,
              );
              executedTests.push({
          ...
          if (executedTests.length) {
            ...
            if (!testResults.summary) {
              testResults.summary = runMode === 'research'
                ? 'Мнение верификатора получено, реальные команды не запускались.'
            ...
            if (reworkResult.success) {
              ...
              if (reworkFiles.length && teamConfig.run?.applyChanges !== false) {
                for (const fileChange of reworkFiles as Array<{ path: string; action: string; content?: string; description?: string; patches?: Array<{ search: string; replace: string }> }>) {
                  ...
                  if (res.ok) {
                    const absolutePath = this.resolveToAbsolutePath(projectPath, fileChange.path);
                    if (absolutePath) appliedFilePaths.push(absolutePath);
                  } else {
        ...
        if (pmWorked || developerWorked || plan.roles.tester.enabled) {
          await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'working', 'Обновляю память проекта');
          ...
          await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'done', 'Память проекта обновлена');
        }

        // 6. PROJECT MANAGER - финальный отчет
        await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'working', 'Формирую итоговый отчет');
        ...
        if (!finalReport.success && finalReport.rawResponse) {
          const fixed = this.tryFixAgentJson(finalReport.rawResponse, 'pm');
          if (fixed) {
        ...
        await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'done', 'Работа завершена');
        ...
        await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'error', `Попытка ${attempt}/${MAX_RETRIES} не удалась: ${lastError}`);
        ...
        if (attempt === MAX_RETRIES) {
          ...
          // Финальное сообщение PM с итоговой причиной
          await this.broadcastActivity(runId, chatId, 'pm', pmName, pmLabel, 'error', `Не получилось выполнить за ${MAX_RETRIES} попыток. Последняя ошибка: ${lastError}`);
          break;
        }
  ...
  private normalizeExecutionPlan(
    ...
    const wantsVerificationPass = /\b(подтверд|подтвержд|верифиц|перепроверь|вторая проверка|second opinion|double-check|validate|verify|проверь фактами)\b/i.test(task);
    const wantsOnlyOpinion = /\b(мнение|opinion|что думаешь|what do you think|оцени|evaluate|compare)\b/i.test(task);
    const hasConcreteFileHint =
      Array.isArray(rawPlan.files) && rawPlan.files.some((item) => !!item && typeof item === 'object' && typeof (item as any).path === 'string' && String((item as any).path).trim().length > 0);
    const localizedImplementationHint =
      hasConcreteFileHint
      || /\b(страниц|page|view|component|компонент|файл|module|модул|endpoint|route|workspaceview|chatservice|runsservice)\b/i.test(task);
    ...
    const defaultAnalystEnabled =
      isDiagnostics
      || isResearch
      || (runMode === 'implementation' && !localizedImplementationHint && (
        /\b(исслед|проанализ|разбер|найд|root cause|investigat|analy[sz]e|diagnos|архитект|спроект|design)\b/i.test(task)
        || steps.some((step) => /анализ|диагноз|исслед|research|spec|design|архитект/i.test(step))
      ));
    const defaultDeveloperEnabled =
      runMode === 'implementation'
      && !/\b(summary|plan only)\b/i.test(task);
    const defaultTesterEnabled =
      (defaultDeveloperEnabled && (
        testingCommands.length > 0
        || localizedImplementationHint
        || /\b(исправ|fix|баг|bug|ошибк|500|socket|ws|build|сборк|провер)\b/i.test(task)
      ))
      || /\b(тест|validate|verify|check|верифиц)\b/i.test(task)
      || steps.some((step) => /тест|провер|validate|verify|check/i.test(step));

    const defaultReviewerEnabled =
      runMode === 'implementation'
      && /\b(ревью|review|проверь код|code review|посмотри код|прочитай код|critical review|critique)\b/i.test(task);

    const roles: Record<ExecutionRole, RoleExecutionPlan> = {
      pm: resolveRole(
        'pm',
        isDiagnostics || isResearch
          ? 'Изучить реальный код, сжать контекст и вернуть только факты по запросу.'
          : 'Разобрать задачу по реальному коду и подготовить короткое точечное ТЗ только по нужным изменениям.',
        defaultAnalystEnabled,
        defaultAnalystEnabled ? 'Нужен сжатый разбор кода перед выполнением.' : 'Задача уже достаточно локализована, отдельный анализ не нужен.',
      ),
      developer: resolveRole(
        'developer',
        isDiagnostics
          ? 'НЕ вносить правок в код. Только подтвердить выводы аналитика. Вернуть SUMMARY: Нет изменений.'
          : 'Работать только по реальным файлам проекта и внести точечные изменения без самодеятельности и лишних файлов.',
        defaultDeveloperEnabled,
        defaultDeveloperEnabled ? 'Нужны реальные изменения в проекте.' : 'Код менять не требуется.',
      ),
      reviewer: resolveRole(
        'reviewer',
        'Провести критический просмотр изменений только при повышенном риске: проверить архитектурные и поведенческие риски.',
        defaultReviewerEnabled,
        defaultReviewerEnabled ? 'Запрошен отдельный критический просмотр изменений.' : 'Критический просмотр не нужен для этой задачи.',
      ),
      tester: resolveRole(
        'tester',
        isDiagnostics
          ? 'Проверить состоятельность диагноза и подтвердить фактами, что он объясняет наблюдаемое поведение.'
          : testingCommands.length
              ? `Проверить изменения фактами и опереться на команды: ${testingCommands.join(', ')}`
              : 'Проверить изменения фактами, командами и точными замечаниями без догадок.',
        defaultTesterEnabled,
        defaultTesterEnabled ? 'Нужна фактическая верификация результата.' : 'Отдельная верификация сейчас не добавит сигнала.',
      ),
    };
    ...
    if (isResearch) {
      ...
      const allowAnalystOpinion = explicitRoleRequest === 'pm' || !explicitRoleRequest;
      roles.pm.enabled = allowAnalystOpinion;
      roles.pm.assignment = allowAnalystOpinion
        ? (wantsOnlyOpinion
            ? 'Изучить код и дать сжатое мнение строго по запросу, без новых задач и без правок.'
            : 'Изучить код и дать точный сжатый ответ строго по запросу, без новых задач и без правок.')
        : '';
      roles.pm.reason = allowAnalystOpinion
        ? 'Исследовательская задача по умолчанию решается Project Manager.'
      ...
      roles.tester.assignment = allowTesterOpinion
        ? 'Изучить код и дать мнение верификатора строго по запросу. Никаких правок, файлов и дополнительных задач.'
        : '';
      roles.tester.reason = allowTesterOpinion
        ? 'Пользователь явно попросил мнение верификатора.'
        : 'Это исследовательская задача: верификатор не нужен без явной просьбы о его мнении.';
    }

    if (!roles.pm.enabled && !roles.developer.enabled && !roles.tester.enabled) {
      roles.pm.enabled = true;
      roles.pm.reason = 'Хотя бы один агент должен проверить задачу на содержательность.';
    }
  ...
  private roleLabel(role: ExecutionRole): string {
    ...
    if (role === 'reviewer') return 'критика';
    return 'верификатора';
  }
  ...
  private buildPmStatus(plan: NormalizedExecutionPlan, runMode: RunMode): string {
    if (runMode === 'research') return 'Изучаю код и сжимаю ответ строго по запросу';
    if (runMode === 'diagnostics') return 'Изучаю код и собираю точный диагноз по фактам';
    return plan.roles.developer.enabled ? 'Изучаю код и готовлю короткое точечное ТЗ' : 'Изучаю код и формирую краткий разбор задачи';
  }

  private buildPmDoneStatus(plan: NormalizedExecutionPlan, runMode: RunMode): string {
    if (runMode === 'research') return 'Сжатый разбор готов';
    if (runMode === 'diagnostics') return 'Диагноз готов';
    return plan.roles.developer.enabled ? 'Короткое ТЗ готово' : 'Разбор задачи готов';
  }
  ...
  private buildDeveloperStatus(plan: NormalizedExecutionPlan, runMode: RunMode): string {
    ...
    if (!plan.roles.pm.enabled) return 'Работаю напрямую по задаче и только по реальному коду';
    return 'Начинаю реализацию по подтверждённому ТЗ и реальным файлам';
  }
  ...
  private buildTesterStatus(plan: NormalizedExecutionPlan, runMode: RunMode): string {
    if (runMode === 'research') return 'Готовлю мнение верификатора без запуска тестов';
    if (runMode === 'diagnostics') return 'Проверяю состоятельность диагноза по фактам';
    if (plan.roles.developer.enabled) return 'Проверяю изменения фактами и командами';
  ...
  private buildTesterDoneStatus(plan: NormalizedExecutionPlan, runMode: RunMode): string {
    if (runMode === 'research') return 'Мнение верификатора готово';
    if (runMode === 'diagnostics') return 'Диагноз подтверждён';
    if (plan.roles.developer.enabled) return 'Верификация завершена';
  ...
  private detectExplicitRoleRequest(task: string): ExecutionRole | null {
    ...
    if (/\b(мнение pm|спроси pm|pm считает|pm opinion|ask the pm)\b/i.test(t)) return 'pm';
    ...
    if (/\b(мнение тестировщика|спроси тестировщика|тестировщик считает|мнение верификатора|спроси верификатора|верификатор считает|tester opinion|qa opinion|verifier opinion|ask the tester|ask qa|ask the verifier)\b/i.test(t)) return 'tester';
    return null;
  }
  ...
  private async broadcastActivity(
    ...
    try {
      const isConversationalBrief =
        status === 'working'
        && (
          role === 'orchestrator'
          || role === 'pm'
          || (role === 'tester' && /возвращаю задачу разработчику|наш[её]л баги/i.test(message))
        );
      await this.chatsService.addMessage(chatId, 'assistant', message, {
        type: isConversationalBrief ? 'agent-brief' : 'agent-status',
        runId,
        agentRole: role,
        agentName: name,
        agentLabel: label,
   ...
   */
  /**
   * Приводит путь агента к абсолютному виду: если уже абсолютный и существует — вернуть как есть,
   * если относительный — сделать абсолютным через projectPath.
   * Всегда возвращает абсолютный путь.
   */
  private resolveToAbsolutePath(projectPath: string, relOrAbs: string): string {
    const p = String(relOrAbs || '').trim();
    if (!p || isUrlLikePath(p)) return '';
    // Если путь уже абсолютный и файл/директория существует — сразу ок
    if (path.isAbsolute(p) && (fs.existsSync(p) || fs.existsSync(path.dirname(p)))) {
      return p;
    }
    // Пропускаем через relPathWithinProject — она убирает зеркальные префиксы
    // (apps/api/apps/web/... → apps/web/...), обрезает "../", контейнерные префиксы
    const cleaned = relPathWithinProject(projectPath, p, fs.existsSync);
    // Делаем абсолютным: очищенный или исходный путь относительно projectPath
    const base = cleaned || p;
    return path.isAbsolute(base) ? base : path.resolve(projectPath, base);
  }
...
Если правки не нужны — верни только SUMMARY: Нет изменений.`;
  ...
  private async applyFileChange(
    ...
    try {
      ...
      const fullPath = this.resolveToAbsolutePath(projectPath, fileChange.path);
      if (!fullPath) return { ok: true };
      if (hasSuspiciousMirroredPath(fullPath)) {
        return { ok: false, error: `Подозрительный нормализованный путь: ${fullPath}` };
      }
      ...
      if (!applyChanges) {
        this.logger.log(`[dry-run] skip applying ${fileChange.action} to ${fullPath}`);
        return { ok: true };
      }
      ...
      if (fileChange.action === 'create' || fileChange.action === 'update') {
        ...
        if (fileChange.action === 'create' && fs.existsSync(fullPath)) {
          this.logger.warn(`applyFileChange: action=create for existing file ${fullPath}, forcing to update`);
        ...
        if (fileChange.action === 'update' && patches.length > 0 && fs.existsSync(fullPath)) {
          ...
          for (const p of patches) {
            ...
            if (current.includes(p.search)) {
              ...
              this.logger.warn(`Patch search block not found in ${fullPath}; skipping that patch.`);
            }
          }
  ...
  /**
   * Определяет, что путь относится к документации, чтобы:
   * 1) отправлять понятные статусы в чат,
   * 2) синхронизировать содержимое в project memory/knowledge graph,
   * 3) при этом НЕ блокировать операции с документацией в репозитории.
   */
  ...
  private validateDeveloperPatches(
    ...
    try {
      ...
      for (const f of files) {
        ...
        const fullPath = this.resolveToAbsolutePath(projectPath, p);
        if (!fullPath) continue;
        if (!fs.existsSync(fullPath)) {
...
5. Документацию (.md/README/docs) можно менять, если это требуется задачей.`;
  }
...
Если вернёшь невалидный JSON — весь запуск упадёт.`;
  ...
  private buildProjectManagerPrompt(run: Run, plan: NormalizedExecutionPlan, project: any, messages: any[], projectPath: string, workspace: any, runMode: RunMode, memoryContext = ''): string {
    ...
    if (runMode === 'research') {
      return `Ты — Аналитик. РЕЖИМ: ИССЛЕДОВАНИЕ / МНЕНИЕ. Твоя задача — ответить СТРОГО на тот вопрос, который поставил пользователь. Ты не управляешь командой и не придумываешь следующий шаг. Никаких дополнительных задач, улучшений, проверок, рефакторов или скрытых рекомендаций сверх запроса.

ПРОЕКТ: ${project.name || 'Unknown'}
РАБОЧАЯ ДИРЕКТОРИЯ: ${projectPath}
ЗАДАЧА: ${run.task}
ПЛАН ОРКЕСТРАТОРА: ${JSON.stringify(plan, null, 2)}
      НАЗНАЧЕНИЕ ДЛЯ ТЕБЯ: ${plan.roles.pm.assignment}
...
Схема:
{"feature":"string","description":"string - прямой ответ по сути запроса","opinion":"string - сжатый вывод по запросу","evidence":[{"file":"string","location":"string","note":"string"}],"risks":["string"],"recommendations":["string - только если прямо следуют из запроса, без новых задач"]}

Если вернёшь невалидный JSON — запуск упадёт.`;
    }

    if (runMode === 'diagnostics') {
      return `Ты — Аналитик. Проведи ДИАГНОСТИКУ кода проекта и найди причину. РЕЖИМ: только анализ, БЕЗ правок кода. Ты должен сжать реальную причину, а не строить широкий отчёт.
...
4. Если задача требует, добавляй/обновляй/удаляй документацию (README, .md/.mdx/.rst, docs/).
...
${memoryContext ? `\nПАМЯТЬ ПРОЕКТА ИЗ БД (используй как справку по реальному проекту, но не как повод придумывать новые задачи):\n${memoryContext}` : ''}
...
9. Если задача требует обновить документацию (.md/README/docs/*), выполняй это обычными FILE-блоками (create/update/delete). После изменений синхронизируй с памятью проекта и knowledge graph.
...
15. Не выполняй скрытых подзадач. Не создавай новые файлы, папки, вспомогательные слои, тесты, документацию или рефакторы, если этого нет в ТЗ и это не требуется напрямую для правки.
16. Работай только с кодом из этого проекта. Если путь не подтверждён текущим кодом или списком файлов, не придумывай его.
...
Лимиты: макс ${workspace.maxFiles} файлов, до ${workspace.maxCharsPerFile} символов на файл.`;
  ...
  private buildTesterPrompt(
    ...
    const failedFiles = Array.isArray((codeChanges as any)?.failedFiles)
      ...
      : [];
    const modeLine = runMode === 'research'
      ? 'РЕЖИМ: исследование. Дай мнение верификатора по вопросу пользователя без запуска тестов и без новых задач.'
      : runMode === 'diagnostics'
        ? 'РЕЖИМ: диагностика. Кода могло не быть вовсе — твоя задача подтвердить или опровергнуть сам диагноз фактами.'
        : 'РЕЖИМ: реализация. Проверь изменения кода фактами и командами.'
    ;
    return `Ты — Верификатор. Проверь результат работы команды на корректность и предложи, какие реальные команды нужно выполнить.
...
Твоя задача — понять, решает ли результат задачу, и предложить краткий набор реальных проверок. Ты не рассуждаешь абстрактно: только наблюдаемые риски, факты и команды. Команды выполняет сервер, не ты.
...
7. Если видишь баг, несовпадение с ТЗ или неприменённые патчи — явно пиши это в "errors". Эти ошибки будут использованы для возврата задачи разработчику.
8. Не пиши общие фразы вроде "нужна дополнительная проверка" без конкретики. Либо назови точный риск, либо оставь массив пустым.
...
{"passed":true,"summary":"Предлагаю grep для проверки наличия функционала репликации пациента в бэкенде.","tests":[{"name":"Поиск термина репликации пациента","command":"rg -n 'replica' apps/api","success":true,"output":"Проверит, есть ли вообще упоминания репликации пациента в бэкенде"}],"errors":[]}

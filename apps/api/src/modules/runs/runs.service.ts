    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        const finalReport = await this.callAgentStream(
          runId, chatId, 'orchestrator', orchestratorAgent, language,
          this.buildFinalReportPrompt(run, plan, spec, codeChanges, testResults, memoryUpdate, runMode, project),
...
3. ${projectUnavailable ? `ВНИМАНИЕ: код проекта НЕДОСТУПЕН (путь не существует или пуст). НЕ выдумывай архитектуру/стек. В SUMMARY честно напиши: "Не могу прочитать код проекта (путь ${project.localPath || ''} недоступен) — примонтируйте проект, тогда дам оценку."` : `Если данных недостаточно для ответа — так и скажи, не придумывай.`}
...
Если команд не нужно — верни {"passed":true,"summary":"","tests":[],"errors":[]}.`;
  ...
  private buildFinalReportPrompt(run: Run, plan: any, spec: any, codeChanges: any, testResults: any, memoryUpdate: any, runMode: RunMode, project: any = null): string {
...
ЗАДАЧА: ${run.task}
${modeDirective}
ПРОЕКТ: ${project?.name || 'Unknown'} (путь: ${project?.localPath || ''})

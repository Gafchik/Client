...
Для текущего MVP операторский интерфейс должен быть **русскоязычным по умолчанию**. Все базовые пользовательские элементы первого контура обязаны использовать русский язык:

- названия экранов;
- статусы этапов;
- подписи полей и кнопок;
- сообщения об ошибках;
- названия артефактов и summary-блоков.

Английский язык допустим только внутри:

- имён архитектурных модулей и канонических терминов, если они уже утверждены документами;
- технических идентификаторов;
- содержимого исходного кода и артефактов проекта, если они извлекаются как данные.
...
На текущем implementation этапе допустим только `safe execution preview`, который:

- показывает разрешённые и запрещённые действия;
- фиксирует необходимость `reindex`, `graph refresh`, `knowledge refresh`;
- не изменяет проект фактически.
...
Для implementation-уровня MVP также допустим viewer compatibility mode:

- старые run-артефакты могут быть автоматически нормализованы до новой UI-формы;
- отсутствие новых полей в исторических артефактах не должно ломать operator console.

### Язык интерфейса MVP

Для первого пользовательского контура принимается следующее обязательное правило:

- основной язык UI: русский;
- язык operator console по умолчанию: русский;
- новые UI-экраны в MVP не должны добавляться только на английском;
- если используется смешанная терминология, человеко-читаемая часть интерфейса должна оставаться на русском.
...
7. Preview-only execution layer до старта controlled mutation runtime.
8. Lightweight compatibility normalization для старых локальных run-артефактов.

...
**Статус:** Частично реализовано; документ фиксирует target UX, текущий UI покрывает не всё  
...
- chat-first экран
- отдельные страницы `Projects`, `Providers`, `Teams`
- блокировка части настроек во время активного run
- compact status polling и отдельный inspector/detailed view для run state

Пока ещё требует шлифовки:

- более "человеческая" подача ответов без лишней служебной сухости
- более явный long-running agent state для очень больших проектов
- дополнительная чистка secondary-метаинформации в transcript
